...
DEFAULT_NAME = "MyPlace"

def createUniqueId (hass: HomeAssistant, name: str, service: str) -> str:
    
        return generate_entity_id  (
            entity_id_format = WEATHER_DOMAIN + '.{}',
            name = "forecast "+ name + " " + service,
            hass = hass
        )
...
async def async_setup_entry( hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback ) -> None:
    ...
    coordinator: MoreWeatherDataUpdateCoordinator = hass.data[DOMAIN][config_entry.entry_id]
    async_add_entities(
        [
            MoreWeatherEntity(coordinator, config_entry.data, hass.config.units is METRIC_SYSTEM, False, hass),
            MoreWeatherEntity(coordinator, config_entry.data, hass.config.units is METRIC_SYSTEM, True, hass)
        ]
    )
...
class MoreWeatherEntity(CoordinatorEntity[MoreWeatherDataUpdateCoordinator], WeatherEntity):
    ...
    def __init__( self, coordinator: MoreWeatherDataUpdateCoordinator, config: Mapping[str, Any], is_metric: bool, hourly: bool, hass: HomeAssistant) -> None:
        """Initialise the platform with a data instance and site."""
        super().__init__(coordinator)
        self._config = config
        self._is_metric = is_metric
        self._hourly = hourly
        self.entity_id = createUniqueId (hass, self.name, self._config[CONF_SERVICE])
    ...
    def unique_id(self) -> str:
        return createUniqueId(self.hass, self.name, self._config[CONF_SERVICE])
    ...
    def device_info(self) -> DeviceInfo:
        """Device info."""
        serviceDef = SERVICEPROVIDERS_MAP[self._config.get(CONF_SERVICE)]

        return DeviceInfo(
            name=f"{serviceDef['name']} forecast",
            entry_type=DeviceEntryType.SERVICE,
            identifiers={(DOMAIN,)},  # type: ignore[arg-type]
            manufacturer=serviceDef["name"],
            model="Forecast",
            configuration_url=serviceDef["website"],
        )
